...
class globals:
            ...
            if section != "USER DEFINED UNITS":
                for key, val in self.config_file.items(section):
                    ...
                    # Check if the first element is a letter (the priors)
                        ...
                        input_prior = [
                            u.Quantity(val[i]).value for i in range(1, 3)
                        ]
                        # try:
                        input_truth = (
                            u.Quantity(val[4]).value
                            if (len(val) >= 5 and val[3] == "T")
                            else None
                        )
                        # except:
                        #    input_truth = None
                        ...
                        conv_unit = self.units.return_units(
                            key, self.units.retrieval_units
                        )
                        conv_truth, conv_prior = self.units.unit_conv(
                            key,
                            input_unit,
                            conv_unit,
                            input_truth,
                    ...
                    # The parameters that are fixed during the retrieval
                        ...
                        conv_truth = self.units.unit_conv(
                            key, input_unit, conv_unit, input_truth
                        )
    ...
    def init_rt(self):
        ...
        for key in self.params.keys():
            if (
                self.params[key]["type"] == "CLOUD PARAMETERS"
        ...
        for key in self.knowns.keys():
            if (
                self.knowns[key]["type"] == "CLOUD PARAMETERS"
                and len(key.split("_")) == 2
            ):
                cloud_dir = str(
                    self.path_opacity
        ...
        self.MMW_Storage = {}
        reader = np.loadtxt(
            self.path_opacity + "/opa_input_files/Molecular_Weights.txt",
            dtype="str",
        )
        ...
        self.rt_object = self.rt.Radtrans(
            line_species=ls,  # sorted(used_line_species),
    ...
    def log_likelihood(self, cube):
        ...
        if self.settings["parametrization"] == "madhuseager":
            if (
                ...
                < self.temp_vars["log_P1"]
                < self.temp_vars["log_P2"]
                < self.temp_vars["log_P3"]
                < self.phys_vars["log_P0"]
            ):
                return -1e32
    ...
    def get_param_sample(self, cube):
        ...
        for par in range(len(r_params)):
            if self.params[r_params[par]]["type"] == "TEMPERATURE PARAMETERS":
                ...
                if (
                    not "_".join(r_params[par].split("_", 2)[:2])
                    in self.cloud_vars.keys()
                ):
                    self.cloud_vars[
                        "_".join(r_params[par].split("_", 2)[:2])
                    ] = {}
                try:
                    self.cloud_vars["_".join(r_params[par].split("_", 2)[:2])][
                        r_params[par].split("_", 2)[2]
                    ] = cube[par]
                except:
                    self.cloud_vars["_".join(r_params[par].split("_", 2)[:2])][
                        "abundance"
    ...
    def make_press_temp_terr(
        self, log_ground_pressure=None, layers=100, log_top_pressure=None
    ):
        """
        ...
        if self.settings["parametrization"] == "polynomial":
            if log_ground_pressure is None:
                self.press = np.logspace(
                    log_top_pressure, self.phys_vars["log_P0"], layers, base=10
                )
            else:
                self.press = np.logspace(
                    log_top_pressure, log_ground_pressure, layers, base=10
                )
            self.temp = np.polyval(
                np.array(
                    [
                        self.temp_vars["a_" + str(len(self.temp_vars) - 1 - i)]
                        for i in range(len(self.temp_vars))
                    ]
                ),
                np.log10(self.press),
            )
            ...
            self.temp = self.vae_pt.get_temperatures(
                z=np.array(
                    [
                        self.temp_vars["z_" + str(i + 1)]
                        for i in range(len(self.temp_vars))
                    ]
                ),
                log_p=np.log10(self.press),
            )
            ...
            self.temp = self.nc.guillot_modif(
                self.press,
                1e1 ** self.temp_vars["log_delta"],
                1e1 ** self.temp_vars["log_gamma"],
                self.temp_vars["t_int"],
                self.temp_vars["t_equ"],
                1e1 ** self.temp_vars["log_p_trans"],
                self.temp_vars["alpha"],
            )
            ...
            P0, P1, P2, P3 = (
                10**log_top_pressure,
                10 ** self.temp_vars["log_P1"],
                10 ** self.temp_vars["log_P2"],
                10 ** self.temp_vars["log_P3"],
            )
            ...
            T2 = (
                self.temp_vars["T0"]
                + (np.log(P1 / P0) / self.temp_vars["alpha1"]) ** (1 / beta1)
                - (np.log(P1 / P2) / self.temp_vars["alpha2"]) ** (1 / beta2)
            )
