...
def Corner(
    ...
    fig, axs = plt.subplots(
        dimension, dimension, figsize=(dimension * 2.5, dimension * 2.5)
    )
    ...
    for param in params:
        ...
        if ULU is not None:
            if param in ULU:
                h = np.histogram(
                    data[param], density=True, bins=bins, range=(ULU_lim[0], 0)
                )
                h2 = np.histogram(
                    np.log10(1 - 10 ** (np.arange(-7, 0, 0.000001))),
                    density=True,
                    bins=h[1],
                )
                h = (h[0] / h2[0], h[1])
        ...
        if quantiles1d is not None:
            if ULU is not None:
                if i in ULU:
                    ...
                    ind = [
                        np.min(np.where(h_cumulative >= quantiles1d[i]))
                        for i in range(len(quantiles1d))
                    ]
                    q = ((h[1][1:] + h[1][:-1]) / 2)[ind]
                else:
                ...
                if add_table:
                    ...
                    table.append(
                        [
                            str(
                                np.round(
                                    truths[param], int(-np.floor(round - 0.5))
                                )
                            ),
                            str(np.round(q[1], int(-np.floor(round - 0.5))))
                            + r" $_{\,"
                            + str(
                                np.round(
                                    q[0] - q[1], int(-np.floor(round - 0.5))
                                )
                            )
                            + r"}^{\,+"
                            + str(
                                np.round(
                                    q[2] - q[1], int(-np.floor(round - 0.5))
                                )
                            )
                            + r"}$",
                        ]
                    ...
                    axs[i, i].set_title(
                        ...
                        + r" $_{\,"
                        + str(
                            np.round(q[0] - q[1], int(-np.floor(round - 0.5)))
                        )
                        + r"}^{\,+"
                        + str(
                            np.round(q[2] - q[1], int(-np.floor(round - 0.5)))
                        )
                        + r"}$",
                        fontsize=fs,
                    )
                ...
                if i > j:
                    ...
                    if param_i in ULU:
                        ...
                        h_new = axs[j, j].hist(
                            ...
                            X,
                            weights=sp.ndimage.filters.gaussian_filter(
                                np.sum(Z, axis=1), [ULU_lim[1]], mode="reflect"
                            ),
                            histtype="stepfilled",
                            color=color_levels[1],
                            density=True,
                        )
                        fac = h_new[0] / sp.ndimage.filters.gaussian_filter(
                            np.histogram(data[param_j], density=True, bins=X)[
                                0
                            ],
                            [ULU_lim[1]],
                            mode="reflect",
                        ...
                        h_cumulative = np.array(
                            [
                                sum(h_new[0][: i + 1]) / sum(h_new[0])
                                for i in range(len(h_new[0]))
                            ]
                        )
                        ind = [
                            np.min(np.where(h_cumulative >= quantiles1d[i]))
                        ...
                        # Round q and print the retrieved value above the histogram plot
                        round = min(
                            np.log10(abs(q[2] - q[1])),
                            np.log10(abs(q[0] - q[1])),
                        )
                        if round >= 0.5:
                            if add_table:
                                table[j] = [
                                    str(int(truths[param_j])),
                                    str(int(q[1]))
                                    + r" $_{\,"
                                    + str(int(q[0] - q[1]))
                                    + r"}^{\,+"
                            ...
                            if add_table:
                                table[j] = [
                                    str(
                                        np.round(
                                            truths[param_j],
                                            int(-np.floor(round - 0.5)),
                                        )
                                    ),
                                    str(
                                        np.round(
                                            q[1], int(-np.floor(round - 0.5))
                                        )
                                    )
                                    + r" $_{\,"
                                    + str(
                    ...
                    if param_j in ULU:
                        ...
                        fac = h_new[0] / sp.ndimage.filters.gaussian_filter(
                            np.histogram(data[param_i], density=True, bins=Y)[
                                0
                            ],
                            [ULU_lim[1] / 2],
                            mode="reflect",
    ...
    for param_i in params:
        ...
        for param_j in params:
            ...
            yticks = [
                (1 - pos) * ylim[0] + pos * ylim[1]
                for pos in [0.2, 0.4, 0.6, 0.8]
            ]
            xticks = [
                (1 - pos) * xlim[0] + pos * xlim[1]
                for pos in [0.2, 0.4, 0.6, 0.8]
            ]
            ...
            if i > j:
                ...
                Z = sp.ndimage.filters.gaussian_filter(
                    Z, [1.5 * ULU_lim[1], 1.5 * ULU_lim[1]], mode="reflect"
                )
            ...
            if j == 0 and i != 0:
                ...
                if roundy >= 0.5:
                    axs[i, j].set_yticklabels(
                        [int(i) for i in yticks], fontsize=fs, rotation=45
                    )
                else:
                    axs[i, j].set_yticklabels(
            ...
            if i == dimension - 1:
                ...
                if roundx >= 0.5:
                    ...
                    axs[i, j].set_xticklabels(
                        np.round(xticks, int(-np.floor(roundx - 0.5))),
                        fontsize=fs,
                        rotation=45,
                        ha="right",
                    )
    ...
    if add_table:
        ...
        the_table = ax_table.table(
            ...
            colColours=colours,
            colLabels=columns,
            bbox=(0.8, 1 - dimension * 0.4 / 17, 0.2, dimension * 0.4 / 17),
        )
...
def Posterior(
    ...
    if ax is None:
        ...
        if quantiles1d is not None:
            if ULU:
                h_cumulative = np.array(
                    [sum(h[0][: i + 1]) / sum(h[0]) for i in range(len(h[0]))]
                )
                ind = [
                    np.min(np.where(h_cumulative >= quantiles1d[i]))
                    for i in range(len(quantiles1d))
            ...
            ax.vlines(
                q, ax.get_ylim()[0], ax.get_ylim()[1], colors="k", ls="--"
            )
            ...
            if round >= 0.5:
                ...
                ax.set_title(
                    ...
                    + str(np.round(q[1], int(-np.floor(round - 0.5))))
                    + r" $_{\,"
                    + str(np.round(q[0] - q[1], int(-np.floor(round - 0.5))))
                    + r"}^{\,+"
                    + str(np.round(q[2] - q[1], int(-np.floor(round - 0.5))))
                    + r"}$"
                )
    ...
    if ax is None:
        # Setting 4 even ticks over the range defined by the limits of the subplot
        xticks = [
            (1 - pos) * xlim[0] + pos * xlim[1] for pos in [0.2, 0.4, 0.6, 0.8]
        ]
        ...
        if roundx >= 0.5:
            ax.set_xticklabels(
                [int(i) for i in xticks], rotation=45, ha="right"
            )
        else:
            ax.set_xticklabels(
                np.round(xticks, int(-np.floor(roundx - 0.5))),
