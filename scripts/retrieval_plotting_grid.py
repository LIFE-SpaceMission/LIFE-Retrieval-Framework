...
class grid_plotting:
    def __init__(self, results_directory, plots_dir="", object_info=None):
        ...
        for model_item in directories:
            ...
            self.grid_results["rp_object"][model_item] = rp.retrieval_plotting(
                model_item
            )
            ...
            if object_info is None:
                self.grid_results["item_classification"][model_item][
                    ...
                    self.grid_results["rp_object"][
                        model_item
                    ].config_file.items("LAMBDA RANGE")[0][1]
                    + "-"
                    + self.grid_results["rp_object"][
                        model_item
                    ].config_file.items("LAMBDA RANGE")[1][1]
                )
                self.grid_results["item_classification"][model_item]["R"] = [
                    i[1]
                    for i in self.grid_results["rp_object"][
                        model_item
                    ].config_file.items("CHEMICAL COMPOSITION PARAMETERS")
                    if i[0] == "settings_resolution"
                ][0]
                self.grid_results["item_classification"][model_item]["SN"] = [
                    i[2:]
                    for i in model_item.split("/")[-2].split("_")
                    if "SN" in i
                ][0]
                self.grid_results["item_classification"][model_item][
                    "Model"
        ...
        for category in self.categories:
            if category in ["R", "SN"]:
                self.sub_categories[category] = sorted(
                    set(
                        [
                            int(
                                self.grid_results["item_classification"][i][
                                    category
                                ]
                            )
                            for i in self.grid_results[
                                "item_classification"
                            ].keys()
                        ]
                    )
                ...
                self.sub_categories[category] = [
                    str(i) for i in self.sub_categories[category]
                ]
            else:
                self.sub_categories[category] = sorted(
                    set(
    ...
    def _grid_generator(
        ...
        if overplot_category is None:
            ...
            local_categories = [
                i for i in local_categories if i != overplot_category
            ]
        ...
        combinations_local_sub_categories = [
            dict(zip(keys, v)) for v in itertools.product(*values)
        ]

        # Get lists of the runnames for each combination of category
        run_categorization = np.zeros(
            (len(combinations_local_sub_categories), y_dim, x_dim, o_dim),
            dtype=object,
        )
        for ind in range(len(combinations_local_sub_categories)):
            for run in self.grid_results["rp_object"].keys():
                equal = [
                    self.grid_results["item_classification"][run][key]
                    == combinations_local_sub_categories[ind][key]
                    for key in combinations_local_sub_categories[ind].keys()
                ]
                if all(equal):
                    if x_category is not None:
                        x_ind = [
                            ...
                            if x_cat[i]
                            == self.grid_results["item_classification"][run][
                                x_category
                            ]
                        ][0]
    ...
    def _get_posterior_data(
        ...
        for run in self.grid_results["rp_object"].keys():
            ...
            for i in names:
                if (
                    self.grid_results["rp_object"][run].params[names[i]]["type"]
            ...
            local_grid_results[run]["local_equal_weighted_post"] = np.copy(
                self.grid_results["rp_object"][run].equal_weighted_post[:, :-1]
            )
            local_grid_results[run]["local_truths"] = self.grid_results[
                "rp_object"
            ][run].truths.copy()
            local_grid_results[run]["local_titles"] = self.grid_results[
                "rp_object"
            ][run].titles.copy()
            local_grid_results[run]["local_posterior_keys"] = list(
                self.grid_results["rp_object"][run].params.keys()
            )
            local_grid_results[run]["local_priors"] = list(
                self.grid_results["rp_object"][run].priors
            )

            # Adust the local copy of the posteriors according to the users desires
            (
                local_grid_results[run]["local_equal_weighted_post"],
                local_grid_results[run]["local_truths"],
                local_grid_results[run]["local_titles"],
            ) = self.grid_results["rp_object"][run].Scale_Posteriors(
                local_grid_results[run]["local_equal_weighted_post"],
                local_grid_results[run]["local_truths"],
                local_grid_results[run]["local_titles"],
            ...
            if plot_bond is not None:
                A_Bond_true, T_equ_true = self.grid_results["rp_object"][
                    run
                ].Plot_Ret_Bond_Albedo(
                    *plot_bond[:-2],
                ...
                local_grid_results[run][
                    "local_equal_weighted_post"
                ] = np.append(
                    local_grid_results[run]["local_equal_weighted_post"],
                    self.grid_results["rp_object"][run].ret_opaque_T,
                    axis=1,
                )
                local_grid_results[run][
                    "local_equal_weighted_post"
                    ...
                    local_grid_results[run]["local_equal_weighted_post"],
                    self.grid_results["rp_object"][run].A_Bond_ret,
                    axis=1,
                )
                local_grid_results[run]["local_truths"] += [
                    T_equ_true,
                ...
                local_grid_results[run]["local_posterior_keys"] += [
                    "T_eq",
                    "A_B",
                ]
    ...
    def posteriors_custom_grid(
        self,
        x_category=None,
        y_category=None,
        overplot_category=None,
        overplot_identifiers=None,
        posterior_identifiers=None,
        subfig_size=[2.5, 3],
        sharex=True,
        sharey=True,
        log_pressures=True,
        ...
        (
            ...
            overplot_category=overplot_category,
            return_all=True,
        )

        if collapse:
            print(x_dim, y_dim)
            run_categorization_collapsed = np.zeros(
                (
                    len(combinations_local_sub_categories),
                    y_dim * x_dim,
                    1,
                    o_dim,
                ),
            ...
            for p in range(len(combinations_local_sub_categories)):
                for i in range(x_dim):
                    run_categorization_collapsed[
                        p, i * y_dim : (i + 1) * y_dim, 0, :
                    ] = run_categorization[p, :, i, :]
            y_dim = x_dim * y_dim
            x_dim = 1
            run_categorization = run_categorization_collapsed.copy()

        # Get all of the unique keys as well as local copies of the posteriors
        local_grid_results, unique_posterior_keys = self._get_posterior_data(
            log_pressures,
            log_mass,
            log_abundances,
            log_particle_radii,
        ...
        for key in unique_posterior_keys:
            ...
            self._posterior_plotting(
                ...
                colors,
                hist_settings,
                overplot_identifiers,
                posterior_identifiers,
                bins,
                subfig_size,
                true_profile,
                key_in=key,
    ...
    def posteriors_grid(
        ...
        (
            ...
            "Posteriors", overplot_category=overplot_category, return_all=True
        )
        ...
        self._posterior_plotting(
            ...
            subfig_size,
            true_profile,
            plotted_posts=plotted_posts,
            ULU_lim=ULU_lim,
        )
        print("Done plotting the posterior grids.")

    def _posterior_plotting(
        self,
        combinations_local_sub_categories,
        run_categorization,
        local_grid_results,
        ...
        for ind in range(len(combinations_local_sub_categories)):
            ...
            fig, ax = plt.subplots(
                y_dim,
                x_dim,
                figsize=(x_dim * subfig_size[0], y_dim * subfig_size[1]),
                sharex=sharex,
                sharey=sharey,
                squeeze=False,
            )
            ...
            for y in range(y_dim):
                for x in range(x_dim):
                    if (key_in is not None) or (
                        (y * x_dim + x) < len(unique_posterior_keys)
                    ):
                        if key_in is None:
                            ...
                            for post in range(
                                ...
                                if (
                                    ...
                                    if hist_settings is None:
                                        ...
                                        hist_setting = hist_settings[
                                            [
                                                i
                                                for i in hist_settings.keys()
                                                if i in run
                                            ][0]
                                        ]
                                    if overplot_identifiers is None:
                                        ...
                                        overplot_identifier = overplot_identifiers[
                                            [
                                                i
                                                for i in overplot_identifiers.keys()
                                                if i in run
                                            ][0]
                                        ]
                                    if posterior_identifiers is None:
                                        ...
                                        posterior_identifier = posterior_identifiers[
                                            [
                                                i
                                                for i in posterior_identifiers.keys()
                                                if i in run
                                            ][0]
                                        ][
                                            post
                                    ...
                                    if (
                                        local_grid_results[run]["local_priors"][
                                            post
                                        ]
                                        == "ULU"
                                    ):
                                        ...
                                        h = ax[y, x].hist(
                                            h[1][:-1],
                                            h[1],
                                            weights=sp.ndimage.filters.gaussian_filter(
                                                h[0],
                                                [ULU_lim[1]],
                                                mode="constant",
                                            ),
                        ...
                        if true_profile:
                            ...
                            if key in df.columns:
                                ...
                                ax2.set_ylim(
                                    df["P(bar)"].max(), df["P(bar)"].min()
                                )
                                ax2.set_ylabel("Pressure (bar)")
                                profiles = "_profile"
                            else:
                                ax[x, y].vlines(
                                    local_grid_results[run_last][
                                        "local_truths"
                                    ][post_last],
                                    ylim[0],
                                    ylim[1],
                        ...
                        # Plot the truth
                        ax[y, x].vlines(
                            local_grid_results[run_last]["local_truths"][
                                post_last
                            ],
                            -10000,
                            10000,
                        ...
                        if (y * x_dim + x) == len(unique_posterior_keys):
                            ...
                            ax[y, x].legend(
                                handles, labels, frameon=False, loc="center"
                            )
                            legend_plotted = True
            ...
            title = "_".join(
                [
                    list(case.keys())[i] + list(case.values())[i]
            ...
            if key_in is None:
                text_ex = fig.text(
                    0.095, 0.4925, "Posterior Density", rotation=90, va="center"
                )
                plt.savefig(
                    save_directory
                    + "/"
                    ...
                    + ".pdf",
                    bbox_inches="tight",
                    bbox_extra_artists=[text_ex],
                )
                plt.clf()
            else:
                # plt.subplots_adjust(top = 0.99, bottom = 0.05, right = 0.96, left = 0.04)
                plt.subplots_adjust(
                    top=0.99, bottom=0.088, right=0.96, left=0.04
                )
            ...
            if not legend_plotted:
                ...
                fig, ax = plt.subplots(
                    1,
                    1,
                    figsize=(0.2, y_dim * subfig_size[1]),
                    sharex=sharex,
                    sharey=sharey,
                    squeeze=False,
                )
                ...
                plt.savefig(
                    save_directory + "/" + title + "/Posterior_ylabel.pdf",
                    bbox_extra_artists=[text_ex],
                )
                plt.clf()
    ...
    def Spectrum_Grid(
        ...
        if type(x_category) == int:
            ...
            (
                ...
                "Spectra", x_category=x_category, y_category=y_category
            )
        ...
        for ind in range(len(combinations_local_sub_categories)):
            ...
            for x in range(x_dim):
                for y in range(y_dim):
                    try:
                        if type(x_category) == int:
                            run = run_categorization[
                                ind, y + x * x_category, 0, 0
                            ]
                        elif type(y_category) == int:
                            run = run_categorization[
                                ind, 0, x + y * y_category, 0
                            ]
                        else:
                        ...
                        if colors is None:
                            ...
                            color = colors[
                                [i for i in colors.keys() if i in run][0]
                            ]
                        if case_identifiers is None:
                            case_identifier = ""
                        else:
                            case_identifier = case_identifiers[
                                [
                                    i
                                    for i in case_identifiers.keys()
                                    if i in run
                                ][0]
                            ]

                        # Plot the spectrum in the corresponding subplot
                        self.grid_results["rp_object"][run].Flux_Error(
                            plot_residual=plot_residual,
                            ax=ax[y, x],
                            save=True,
                            quantiles=quantiles,
                            quantiles_title=quantiles_title,
                            plot_noise=True,
                            plot_true_spectrum=True,
                            legend_loc=legend_loc,
                            color=color,
                            noise_title="LIFE Noise",
            ...
            for i in range(y_dim):
                if plot_residual:
                    ...
                    ax[i, 0].set_ylabel(
                        r"Flux at 10 pc $\left[\mathrm{\frac{erg}{s\,Hz\,m^2}}\right]$"
                    )
            for i in range(x_dim):
            ...
            if plot_residual:
                plt.savefig(
                    save_directory + title + "Spectra_Residual.pdf",
                    bbox_inches="tight",
                )
            else:
                plt.savefig(
                    save_directory + title + "Spectra.pdf", bbox_inches="tight"
                )
    ...
    def PT_Grid(
        ...
        if type(x_category) == int:
            ...
            (
                ...
                "PT_profiles", x_category=x_category, y_category=y_category
            )
        ...
        for ind in range(len(combinations_local_sub_categories)):
                ...
                for x in range(x_dim):
                    try:
                        ...
                        try:
                            ...
                            self.grid_results["rp_object"][run].PT_Envelope_V3(
                                plot_residual=plot_residual,
            ...
            if plot_residual:
                plt.savefig(
                    save_directory + title + "PT_Residual.pdf",
                    bbox_inches="tight",
                )
            else:
                plt.savefig(
                    save_directory + title + "PT.pdf", bbox_inches="tight"
                )
    ...
    def Grid_Model_Comparison(
        ...
        (
            ...
            "Model_Compare",
            x_category=model_category,
            y_category=x_category,
            overplot_category=y_category,
            return_all=True,
        )
        ...
        K_Matrix = (
            np.zeros(
                (
                    (y_dim + 1) * (m_dim - 1) - 1,
                    (x_dim + 1) * len(combinations_local_sub_categories) - 1,
                )
            )
            - 100
        )
        ...
        for ind in range(len(combinations_local_sub_categories)):
            m_corr = 0
            m_compare = [i for i in range(m_dim) if m_cat[i] == model_compare][
                0
            ]
            m_combinations = []
                ...
                if m == m_compare:
                    ...
                    for x in range(x_dim):
                        for y in range(y_dim):
                            ...
                            evidence = self.grid_results["rp_object"][
                                run_categorization[ind, x, m, y]
                            ].evidence
                            evidence_compare = self.grid_results["rp_object"][
                                run_categorization[ind, x, m_compare, y]
                            ].evidence

                            # Calculate the bayes factor and store the result in the matrix
                            K = 0.4342944819 * (
                                float(evidence_compare[0]) - float(evidence[0])
                            )
                            K_Matrix[
                                (y_dim + 1) * (m + m_corr) + y,
                                (x_dim + 1) * (ind) + x,
                        ...
                        m_combinations += [
                            case_identifiers[
                                [
                                    ...
                                    for i in case_identifiers.keys()
                                    if i
                                    in run_categorization[ind, 0, m_compare, 0]
                                ][0]
                            ]
                            + " vs.\n"
                            + case_identifiers[
                                [
                                    i
                                    for i in case_identifiers.keys()
                                    if i in run_categorization[ind, 0, m, 0]
                                ][0]
                            ]
                        ]
        ...
        fig, ax = plt.subplots(
            2,
            figsize=(0.6 * np.shape(K_Matrix)[1], 0.6 * np.shape(K_Matrix)[0]),
            gridspec_kw={
                "height_ratios": [np.shape(K_Matrix)[0] / C_y, 5 / 1.5 / C_y]
            },
        )  # ,linewidth=2*lw, edgecolor="#007272")
        plt.subplots_adjust(hspace=0, wspace=0)
        ...
        ax[0].vlines(
            [-0.5 + i for i in range(np.shape(K_Matrix)[1] + 1)],
            -0.5,
            np.shape(K_Matrix)[0] - 0.5,
            color=facecolor,
            lw=lw,
        )
        ax[0].hlines(
            [-0.5 + i for i in range(np.shape(K_Matrix)[0] + 1)],
            -0.5,
            np.shape(K_Matrix)[1] - 0.5,
            color=facecolor,
            lw=lw,
        )
        ...
        for pos_x in range(n_cases):
            ...
            if x_cat_label is None:
                ax[0].text(
                    ...
                    ha="center",
                    va="center",
                )  # ax[0].text(x_ax[pos_x],-1.35,x_category,rotation = 0,rotation_mode='default',ha='center',va='center')
            else:
                ax[0].text(
                    x_ax[pos_x],
                    -1.35,
            ...
            ax[0].text(
                ...
                -2.15,
                [
                    list(combinations_local_sub_categories[i].keys())[0]
                    + "\n"
                    + list(combinations_local_sub_categories[i].values())[0]
                    + " $\mu$m"
                    for i in range(len(combinations_local_sub_categories))
                ][pos_x],
                rotation=0,
                rotation_mode="default",
                ha="center",
        ...
        for pos_y in range(np.shape(K_Matrix)[0]):
            for pos_x in range(np.shape(K_Matrix)[1]):
                if K_Matrix[pos_y, pos_x] != -100:
                    txt = ax[0].annotate(
                        str(np.round(K_Matrix[pos_y, pos_x], 1)),
                        [pos_x, pos_y + 0.03],
                        ha="center",
                        va="center",
                        color="white",
                    ...
                    if K_Matrix[pos_y, pos_x] < 0:
                        txt.set_path_effects(
                            [
                                PathEffects.withStroke(
                                    linewidth=0.7, foreground="#d62728"
                                )
                            ]
                        )
                    else:
                        txt.set_path_effects(
                            [
                                PathEffects.withStroke(
                                    linewidth=0.7, foreground="#2ca02c"
                                )
                            ]
                        )
        ...
        ax[1].vlines(
            [i - 0.5 for i in range(0, 9)], 0, 4, color=facecolor, lw=2 * lw
        )
        ax[1].vlines(
            [i - 0.5 for i in range(1, 8)], 2.47, 3.53, color="k", lw=lw / 2
        )
        ...
        ax[1].plot(
            [3.635, 7.5 - 0.28], [3.85, 3.85], color="k", ls="-", lw=lw / 2
        )
        ax[1].plot(
            [-0.1, -0.35, -0.1],
            [3.75, 3.85, 3.95],
        ...
        ax[1].text(
            3.5,
            0.8,
            "Color Coding of " + r"$\mathrm{log_{10}(K)}$",
            rotation=0,
            rotation_mode="default",
            ha="center",
            va="center",
        ...
        ax[1].text(
            ...
            case_identifiers[
                [
                    i
                    for i in case_identifiers.keys()
                    if i in run_categorization[ind, 0, m_compare, 0]
                ][0]
            ],
            rotation=0,
            rotation_mode="default",
        ...
        plt.savefig(
            save_directory + "/" + model_compare + "baye_total.pdf",
            bbox_inches="tight",
            facecolor=facecolor,
        )
    ...
    def Parameters_Comparison(
        self,
        parameters=["R_pl", "M_pl", "P_equ", "T_equ", "A_Bond"],
        priors=[True, True, False, False, False],
        units=["$\mathrm{R_\oplus}$", "dex", "dex", "K", ""],
        span=[0.2, 0.5, 0.5, 20, 0.1],
        titles=None,
        bond_params=[1, 0.1, 0.723, 0.0723, 0.77, 226, 5e-2],
        filename=None,
    ):
        ...
        for model in self.model:
            for wln in self.wln:
                ...
                for param in parameters:
                    # Plot the background
                    plt.plot(
                        [2 * count_param + 1, 2 * count_param + 1],
                        [-2, 3],
                        "-k",
                        alpha=1,
                    )
                    ...
                    plt.annotate(
                        r"$-$"
                        + str(span[count_param])
                        + " "
                        + str(units[count_param]),
                        [2 * count_param + 1 - 0.5, -1.15],
                        fontsize=15,
                        rotation=90,
                        va="center",
                        ha="left",
                    ...
                    for snr in sorted(self.snr):
                        # SNR annotation
                        plt.annotate(
                            r"$\mathrm{S/N}=$" + str(snr),
                            [-0.9, 0.32 + count_snr * 0.67],
                            fontsize=15,
                            rotation=0,
                            va="center",
                            ha="left",
                        ...
                        for res in sorted(self.res):
                            ...
                            try:
                                ...
                                keys = list(
                                    self.grid_results[model][wln][res][
                                        snr
                                    ].params.keys()
                                )
                                ind = np.where(np.array(keys) == str(param))[0][
                                    0
                                ]
                                ...
                                if units[count_param] == "dex":
                                    ...
                                    post = self.grid_results[model][wln][res][
                                        snr
                                    ].equal_weighted_post[:, ind]
                                    true = self.grid_results[model][wln][res][
                                        snr
                                    ].truths[ind]
                                prior = self.grid_results[model][wln][res][
                                    snr
                                ].priors_range[ind]
                            ...
                            # What to do If they were not retrieved for
                                ...
                                if param == "A_Bond":
                                    ...
                                    if not hasattr(
                                        self.grid_results[model][wln][res][snr],
                                        "A_Bond_ret",
                                    ):
                                        print()
                                        ...
                                        self.grid_results[model][wln][res][
                                            ...
                                            *bond_params[:4],
                                            A_Bond_true=bond_params[4],
                                            T_equ_true=bond_params[5],
                                            plot=False
                                        )
                                    post = self.grid_results[model][wln][res][
                                        snr
                                    ...
                                    if param in ["T0", "T_eq"]:
                                        if (
                                            ...
                                            == "opaque"
                                        ):  # len(self.grid_results[model][wln][res][snr].cloud_vars) != 0:
                                            post = self.grid_results[model][
                                                wln
                                            ][res][snr].temperature_cloud_top
                                            true = bond_params[5]
                                        else:
                                            post = self.grid_results[model][
                                                wln
                                            ][res][snr].temperature[:, -1]
                                            true = bond_params[5]
                                    else:
                                        if (
                                            self.grid_results[model][wln][res][
                                            ...
                                            post = np.log10(
                                                self.grid_results[model][wln][
                                                    res
                                                ][snr].pressure_cloud_top
                                            )
                                ...
                                # If none of the above cases the variable cannot be plotted
                                    ...
                                    print(
                                        "Error: Parameter "
                                        + str(param)
                                        + " not defined in input file config.ini."
                                    )
                                    sys.exit()
                            ...
                            plt.plot(
                                [
                                    center
                                    - 0.5 * (q50 - q84) / (span[count_param]),
                                    center
                                    - 0.5 * (q50 - q16) / (span[count_param]),
                                ],
                                [offset, offset],
                                ls="-",
                                color=colors[count_res],
                                linewidth=lw,
                            )
                            plt.plot(
                                [
                            ...
                            plt.plot(
                                ...
                                [offset, offset],
                                color=colors[count_res],
                                marker=markers[count_snr],
                                markersize=ms,
                                markeredgecolor="black",
                                markeredgewidth=lw / 4,
                            )
                ...
                lgd = plt.legend(
                    ...
                    handler_map={str: rp_hndl.Handles()},
                    handlelength=4,
                    ncol=2,
                    bbox_to_anchor=(0.25, -0.65, 0.5, 0.5),
                    borderaxespad=0.1,
                    loc="center",
                    fontsize=18,
                )
    ...
    def Parameters_Hist(
        ...
        parameters=[
            ...
            "CO2",
            "CO",
            "H2SO484(c)_am_top_pressure",
            "H2SO484(c)_am",
            "H2SO484(c)_am_thickness",
            "H2SO484(c)_am_particle_radius",
            "H2SO484(c)_am_sigma_lnorm",
            "A_Bond",
        ],
        priors=[True, True, False, False, False],
        units=[
        ...
        span=[0.2, 0.5, 0.5, 20, 0.1],
        titles=None,
        bond_params=[1, 0.05, 0.723, 0.05 * 0.723, 0.77, 226, 5e-2],
        filename=None,
        skip=1,
        n_processes=50,
    ):
        ...
        for model in self.model:
            for wln in self.wln:
                ...
                for param in parameters:
                    ...
                    plt.title(
                        param + " " + str(model) + ", " + str(wln), fontsize=20
                    )
                    # plt.yticks([],fontsize=20,rotation = 90)
                    # xticks = []
                                ...
                                if param == "A_Bond":
                                    ...
                                    if not hasattr(
                                        ...
                                        self.grid_results[model][wln][res][
                                            ...
                                            skip=skip, n_processes=n_processes
                                        )
                                    if param in ["T0", "T_eq"]:
                                        if (
                                            self.grid_results[model][wln][res][
                    ...
                    plt.plot(
                        [true, true], plt.gca().get_ylim(), "k--", label="Input"
                    )
                    plt.ylim(ylim)
                    plt.legend(loc="best")
                    if filename is None:
    ...
    def Parameters_Hist_Top(
        self,
        parameters=["R_pl", "M_pl", "H2O", "CO2", "CO", "H2SO484(c)_am"],
        ranges=[0.1, 0.1, 1, 1, 1, 1],
        units=[
            "$\mathrm{R_\oplus}$",
            "dex",
            "dex",
                ...
                for param in parameters:
                    ...
                    for snr in sorted(self.snr):
                        ...
                        for res in sorted(self.res):  # sorted(self.res):
                            ...
                            if True:  # try:
                                ...
                                if self.grid_results[model][wln][res][
                                    snr
                                ].priors[ind] in ["G", "LG"]:
                                    prior_t = self.grid_results[model][wln][
                                        res
                                    ][snr].priors_range[ind]
                                    prior = [
                                        prior_t[0] - 3 * prior_t[1],
                                        prior_t[0] + 3 * prior_t[1],
                                    ]
                                else:
                                    prior = self.grid_results[model][wln][res][
                            ...
                            if Data_found:
                                m_hi, m_bi = np.histogram(
                                    (post - prior[0]) / (prior[1] - prior[0]),
                                    bins=[
                                        1 / bins * ind
                                        for ind in range(bins + 1)
                                    ],
                                    density=True,
                                )
                                print(m_bi)
                    ...
                    for snr in sorted(self.snr):
                        ...
                        for res in sorted(self.res):
                            ...
                            for k in range(bins):
                                ax[len(self.snr) - 1 - count_snr].fill(
                                    [
                                        m_bins[count][k],
                                        m_bins[count][k + 1],
                                        m_bins[count][k + 1],
                                    ...
                                    color="k",
                                    alpha=(m_hist[count] / maxim)[k],
                                    lw=0,
                                )
                                ax[len(self.snr) - 1 - count_snr].plot(
                                    [0, 1], [count_res, count_res], "k-"
                                )
                            count_res += 1
                            count += 1

                        ax[len(self.snr) - 1 - count_snr].set_yticks(
                            [i + 0.5 for i in range(count_res)]
                        )
                        ax[len(self.snr) - 1 - count_snr].set_yticklabels(
                            ["R " + str(i) for i in sorted(self.res)]
                        )
                        ax[len(self.snr) - 1 - count_snr].plot(
                            [
                                (true - prior[0]) / (prior[1] - prior[0]),
                                (true - prior[0]) / (prior[1] - prior[0]),
                            ],
                            [0, count_res],
                            color="C3",
                        ...
                        ax[len(self.snr) - 1 - count_snr].plot(
                            [
                                (true + ranges[count_param] - prior[0])
                                / (prior[1] - prior[0]),
                                (true + ranges[count_param] - prior[0])
                                / (prior[1] - prior[0]),
                            ],
                        ...
                        ax[len(self.snr) - 1 - count_snr].set_ylim(
                            [0, count_res]
                        )
                        ax[len(self.snr) - 1 - count_snr].set_xticks([])
                        ax[len(self.snr) - 1 - count_snr].set_ylabel(
                            "S/N " + str(snr) + "\n"
                        )
            ...
            for model_compare in self.model:
                if not model == model_compare:
                    ...
                    Mat = (
                        np.zeros(
                            (
                                (len(self.res) + 1) * ((len(self.wln) + 1) // 2)
                                - 1,
                                (len(self.snr) + 1) * ((len(self.wln) + 1) // 2)
                                - 1,
                            )
                        )
                        - 100
                        ...
                        try:
                            ...
                            for snr in sorted(self.snr):
                                ...
                                for res in sorted(self.res):
                                    ...
                                    post_compare = self.grid_results[
                                        model_compare
                                    ][wln][res][snr].evidence

                                    # Calculate the bayes factor and store the result in the matrix
                                    K = 0.4342944819 * (
                                        float(post_compare[0]) - float(post[0])
                                    )
                                    print(
                                        5 * (count_wln // 2) + count_res,
                                        5 * (count_wln % 2) + count_snr,
                                        K,
                                    )
                                    Mat[
                                        5 * (count_wln // 2) + count_res,
                    ...
                    ax.vlines(
                        [-0.5 + i for i in range(np.shape(Mat)[1] + 1)],
                        -0.5,
                        np.shape(Mat)[0] - 0.5,
                        "azure",
                        alpha=1,
                    ...
                    ax.hlines(
                        [-0.5 + i for i in range(np.shape(Mat)[0] + 1)],
                        -0.5,
                        np.shape(Mat)[0] - 0.5,
                        "azure",
                        alpha=1,
                    ...
                    for pos in range(len(y_ax)):
                        for i in range(len(self.snr)):
                            ax.text(
                                y_ax[pos] - (len(self.snr) - 1) / 2.0 + i,
                                -0.7,
                                str(sorted(self.snr)[i]),
                                rotation=0,
                                rotation_mode="default",
                                ha="center",
                                va="center",
                    ...
                    # plt.savefig('Results/Abundances/Summary/'+Wlen_grid[i]+'.png',dpi=450,bbox_inches='tight', facecolor='azure')
                    title = plt.title(
                        "Bayes factor for model " + str(model_compare) + ".",
                        y=1.15,
                    )
                    plt.savefig(
                        self.results_directory
    ...
    def Model_Comparison_Wlen(self):
        ...
        for wln in sorted(self.wln):
            for model in self.model:
                # Define a matrix to store the Bayes factors
                print(
                    (len(self.res) + 1) * ((len(self.model)) // 2) - 1,
                    (len(self.snr) + 1) * 2 - 1,
                )
                Mat = (
                    np.zeros(
                    ...
                    if not model == model_compare:
                        ...
                        for snr in sorted(self.snr):
                            ...
                            for res in sorted(self.res):
                                ...
                                post_compare = self.grid_results[model_compare][
                                    wln
                                ][res][snr].evidence
                                ...
                                print(
                                    5 * (count_model_compare // 2) + count_res,
                                    5 * (count_model_compare % 2) + count_snr,
                                    K,
                                )
                                Mat[
                                    5 * (count_model_compare // 2) + count_res,
                        ...
                        combinations += [
                            str(model) + " vs " + str(model_compare)
                        ]
                        # except:
                        #    pass
                ...
                for pos in range(len(x_ax)):
                    for i in range(len(self.res)):
                        ax.text(
                            -0.6,
                            x_ax[pos] - (len(self.res) - 1) / 2.0 - 0.05 + i,
                            str(sorted(self.res)[i]),
                            rotation=0,
                            rotation_mode="default",
                            ha="right",
    ...
    def Model_Comparison_All(self):
        ...
        for model in sorted(self.model):
            ...
            Mat = (
                np.zeros(
                    (
                        (len(self.res) + 1) * (len(self.model) - 1) - 1 + 3,
                        (len(self.snr) + 1) * len(self.wln) - 1,
                    )
                )
                - 100
            )
                    ...
                    if not model == model_compare:
                        ...
                        for snr in sorted(self.snr):
                            ...
                            for res in sorted(self.res):
                                ...
                                print(
                                    5 * (count_model_compare) + count_res,
                                    5 * (count_model_compare % 2) + count_snr,
                                    K,
                                )
                                Mat[
                                    5 * (count_model_compare) + count_res,
                                    5 * (count_wln) + count_snr,
                                ] = K
                                count_res += 1
                            count_snr += 1
                        count_model_compare += 1
            ...
            ax.vlines(
                [13.55], -0.5, np.shape(Mat)[0] - 0.5, "azure", alpha=1, lw=2
            )
            ax.hlines(
                [-0.5 + i for i in range(np.shape(Mat)[0] + 1)],
                -0.5,
            ...
            for pos in range(len(y_ax)):
                ...
                ax.text(
                    y_ax[pos],
                    -2.2,
                    str(sorted(self.wln)[pos]) + r" $\mu$m",
                    rotation=0,
                    rotation_mode="default",
                    ha="center",
                    va="center",
            ...
            ax.vlines(
                [3.5 + i for i in range(np.shape(Mat)[1] - 7)],
                np.shape(Mat)[0] - 2.5,
                np.shape(Mat)[0] - 1.5,
                color="#007272",
                alpha=1,
            ...
            ax.plot(
                [2.72, 6.365],
                [np.shape(Mat)[0] - 1.25, np.shape(Mat)[0] - 1.25],
                color="#007272",
                ls="-",
                lw=lw,
            )
            ...
            # title = plt.title('Bayes factor for model '+str(model_compare)+'.', y=1.15)
            plt.savefig(
                self.results_directory + "/" + model + "/Baye_Total.pdf",
                bbox_inches="tight",
                facecolor="azure",
    ...
    def Species_Grid(
        self,
        parameters=["H2O", "CO2", "CO", "H2SO484(c)"],
        units=["$\mathrm{R_\oplus}$", "dex", "K", ""],
        span=[0.1, 0.5, 10, 0.1],
        titles=None,
    ):
        # Iterate over models and wavelength ranges
    ...
    def Constraint_Analysis(
        self,
        parameters=["H2O", "CO2", "H2SO484(c)_am", "CO", "R_pl", "M_pl"],
        relative=None,
        units=[
            "dex",
            "dex",
        ...
        for model in self.model:
            for wln in self.wln:
                ...
                for param in parameters[:3]:
                    keys = list(
                        self.grid_results[model][wln][res][snr].params.keys()
                    )
                    ind = np.where(np.array(keys) == str(param))[0][0]
                    truth = np.log10(
                        self.grid_results[model][wln][res][snr].truths[ind]
                    )
                    ...
                    if relative is None:
                        plt.xlabel(
                            r"Abundance $\mathrm{log_{10}}$",
                            fontsize=20,
                            labelpad=20,
                        )
                    else:
                        plt.xlabel(
                            r"Abundance relative to "
                            + str(relative)
                            + r"$\mathrm{log_{10}}$",
                            fontsize=20,
                            labelpad=20,
                    ...
                    plt.plot(
                        [-7, 0],
                        [2 + 2 * (2 - count_param), 2 + 2 * (2 - count_param)],
                        "-k",
                        alpha=1,
                    )
                    plt.plot(
                        [truth, truth],
                        [2 * (2 - count_param), 2 + 2 * (2 - count_param)],
                        "k-",
                    )
                    plt.fill_betweenx(
                        [2 * (2 - count_param), 2 + 2 * (2 - count_param)],
    ...
    def Limit_Plot(
        self, n, N, model, SNR_O, ms, lw, eb, c, m, centre=-3.5, un_c_len=2
    ):
        ...
        if model[0] == "F":
            plt.plot(
                [centre, centre - un_c_len],
                [2 * (N - n) + SNR_O, 2 * (N - n) + SNR_O],
                ls=":",
                color=c,
                linewidth=lw,
            )
        ...
        if model[0] == "SS":
            ...
            plt.plot(
                [HM - un_c_len + 3 * eb, HM - un_c_len, HM - un_c_len + 3 * eb],
                [
                    2 * (N - n) + eb + SNR_O,
                    2 * (N - n) + SNR_O,
                    2 * (N - n) - eb + SNR_O,
        ...
        if model[0] == "SSG":
            ...
            plt.plot(
                [xm - un_c_len + 3 * eb, xm - un_c_len, xm - un_c_len + 3 * eb],
                [
                    2 * (N - n) + SNR_O - eb,
                    2 * (N - n) + SNR_O,
                    2 * (N - n) + SNR_O + eb,
    ...
    def Grid_Model_ComparisonT(
        ...
        K_Matrix = (
            np.zeros(
                (
                    (y_dim + 1) * len(combinations_local_sub_categories) - 1,
                    (x_dim + 1) * (m_dim - 1) - 1,
                )
            )
            - 100
        ...
        fig, ax = plt.subplots(
            ...
            2,
            figsize=(
                0.65 * np.shape(K_Matrix)[1],
                0.65 * np.shape(K_Matrix)[0],
            ),
            gridspec_kw={
                "width_ratios": [np.shape(K_Matrix)[1] / C_y, 6 / 1.5 / C_y]
            },
        )  # ,linewidth=2*lw, edgecolor="#007272")
        plt.subplots_adjust(hspace=0, wspace=0)
        fig.patch.set_facecolor(facecolor)
        ...
        ax[1].hlines(
            [i - 0.5 for i in range(0, 9)], 0, 5, color=facecolor, lw=2 * lw
        )
        ax[1].hlines(
            [i - 0.5 for i in range(1, 8)], 3.47, 4.53, color="k", lw=lw / 2
        )
        ...
        ax[1].plot(
            [4.85, 4.85], [3.635, 7.5 - 0.28], color="k", ls="-", lw=lw / 2
        )
        ax[1].plot(
            [4.75, 4.85, 4.95],
            [-0.1, -0.35, -0.1],
