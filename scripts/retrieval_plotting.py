...
from pyretlife.plotting import (
    ...
    inlay as rp_inlay,
    parallel as rp_parallel,
    posteriors as rp_posteriors,
)
...
class retrieval_plotting(r_globals.globals):
    def __init__(self, results_directory):
        ...
        super().__init__(
            input_file=str(self.results_directory + "input.ini"),
            retrieval=False,
        )
        ...
        self.data = nest.Analyzer(
            self.n_params, outputfiles_basename=self.results_directory
        )
        self.equal_weighted_post = self.data.get_equal_weighted_posterior()
        self.best_fit = self.data.get_best_fit()["parameters"]
        self.evidence = [
            self.data.get_stats()["global evidence"],
            self.data.get_stats()["global evidence error"],
        ]
    ...
    def generate_titles(self):
        ...
        for key in self.params.keys():
            ...
            if self.params[key]["type"] == "CHEMICAL COMPOSITION PARAMETERS":
                self.params[key]["title"] = (
                    "$\\mathrm{"
                    + "_".join(
                        re.sub(r"([0-9])", r" \1", key.split("_")[0]).split()
                    )
                    + "}$"
                )
            ...
            # Define the titles such that they work well for the physical parameters
                ...
                try:
                    ...
                    self.params[key]["title"] = (
                        "$\\mathrm{"
                        + "_".join(re.sub(r"([0-9])", r" \1", key).split())
                        + "}$"
                    )
            ...
            # Define the titles such that they work well for the cloud parameters
                ...
                temp[0] = (
                    "$\\mathrm{"
                    + "_".join(re.sub(r"([0-9])", r" \1", temp[0][:-3]).split())
                    + "}$"
                )
                temp.pop(1)
                self.params[key]["title"] = "\n".join(temp)
        ...
        try:
            self.input_temperature, self.input_pressure = np.loadtxt(
                self.settings["input_profile"]
            ).T
        except:
            pass
    ...
    def Calc_PT_Profiles(
        self, skip=1, layers=500, p_surf=4, n_processes=None, process=None
    ):
        """
        ...
        temp_equal_weighted_post = np.append(
            np.array([self.best_fit]), self.equal_weighted_post[:, :-1], axis=0
        )
        temp_equal_weighted_post = np.append(
            np.array([self.truths]), temp_equal_weighted_post, axis=0
        )
        ...
        process, ind_start, ind_end = rp_parallel.task_assignment(
            "PT-Profile", n_processes, dimension, process
        )
        ...
        for i in range(ind_start, ind_end):
            ...
            for key in self.cloud_vars.keys():
                cloud_bottoms += [
                    self.cloud_vars[key]["top_pressure"]
                    + self.cloud_vars[key]["thickness"]
                ]
                cloud_tops += [self.cloud_vars[key]["top_pressure"]]
                self.make_press_temp_terr(
                    log_top_pressure=np.log10(np.min(cloud_tops)), layers=layers
                )
            ...
            if i == 0:
                ...
                if len(self.cloud_vars) != 0:
                    ...
                    results["true_temperature_cloud_top"] = [
                        temperature_cloud_top
                    ]
            elif i == 1:
                ...
                if len(self.cloud_vars) != 0:
                    ...
                    results["best_temperature_cloud_top"] = [
                        temperature_cloud_top
                    ]
                ...
                if (i == 2) or (i == ind_start):
                    ...
                    results["pressure_extrapol"] = np.zeros(
                        (size, len(pressure_extrapol))
                    )
                    results["temperature_extrapol"] = np.zeros(
                        (size, len(temperature_extrapol))
                    )
                    if len(self.cloud_vars) != 0:
                        results["pressure_cloud_top"] = np.zeros(
                            (size, len([pressure_cloud_top]))
                        )
                        results["temperature_cloud_top"] = np.zeros(
                            (size, len([temperature_cloud_top]))
                        )
            ...
            if process == 0:
                t_end = t.time()
                remain_time = (t_end - t_start) / (
                    (i + 1) / (ind_end - ind_start)
                ) - (t_end - t_start)
                print(
                    "\t"
                    + str(np.round((i + 1) / (ind_end - ind_start) * 100, 2))
                    + " % of PT-profiles calculated. Estimated time remaining: "
                    + str(remain_time // 3600)
                    + " h, "
                    + str((remain_time % 3600) // 60)
                    + " min.        ",
                    end="\r",
                )
    ...
    def Calc_Spectra(self, skip, n_processes=None, process=None):
        ...
        # Split up the jobs onto the multiple processes
        process, ind_start, ind_end = rp_parallel.task_assignment(
            "Spectrum", n_processes, dimension, process
        )
        ...
        for i in range(ind_start, ind_end):
            ...
            if i == 0:
                results["wavelength"] = (
                    self.nc.c / self.rt_object.freq / 1e-4
                )  # self.dwlen[instrument]
                if self.settings["moon"] == "True":
                    results["true_flux"] = np.array(
                        self.rt_object.flux + self.moon_flux
                    )
                else:
                ...
                if (i == 2) or (i == ind_start):
                    ...
                    # Initialize the arrays for storage
                    results["retrieved_fluxes"] = np.zeros(
                        (size, len(self.rt_object.flux))
                    )
                    results["retrieved_em_contr"] = np.zeros(
                        (
                            size,
                            np.shape(self.rt_object.contr_em)[0],
                            np.shape(self.rt_object.contr_em)[1],
                        )
                    )
                ...
                # Save the results
                if self.settings["moon"] == "True":
                    results["retrieved_fluxes"][save, :] = (
                        self.rt_object.flux + self.moon_flux
                    )
                else:
                    results["retrieved_fluxes"][save, :] = self.rt_object.flux
                results["retrieved_em_contr"][
                    save, :, :
                ] = self.rt_object.contr_em
            ...
            if process == 0:
                ...
                print(
                    "\t"
                    + str(np.round((i + 1) / (ind_end - ind_start) * 100, 2))
                    + " % of spectra calculated. Estimated time remaining: "
                    + str(remain_time // 3600)
                    + " h, "
                    + str((remain_time % 3600) // 60)
                    + " min.            ",
    ...
    def get_spectra(self, skip=1, n_processes=50, reevaluate_spectra=False):
        ...
        if not hasattr(self, "retrieved_fluxes"):
            ...
            self.__get_data(
                data_type="Spec",
                data_name="spectra",
                function_name="Calc_Spectra",
                function_args=function_args,
                force_evaluate=reevaluate_spectra,
            )

    def get_pt(
        self, skip=1, layers=500, p_surf=4, n_processes=50, reevaluate_PT=False
    ):
        """
        gets the PT profiles corresponding to the parameter values
        of the equal weighted posteriors.
        ...
        if not hasattr(self, "pressure"):
            ...
            self.__get_data(
                data_type="PT",
                data_name="PT profiles",
                function_name="Calc_PT_Profiles",
                function_args=function_args,
                force_evaluate=reevaluate_PT,
            )
    ...
    def __get_data(
        ...
        # Calculate from scratch
            ...
            if (
                np.shape(self.equal_weighted_post)[0] // function_args["skip"]
            ) // function_args["n_processes"] < 3:
                print("Not enough jobs for the specified number of processes!")
                while (
                    np.shape(self.equal_weighted_post)[0]
                    // function_args["skip"]
                ) // function_args["n_processes"] < 3:
                    function_args["n_processes"] -= 1
                print(
                    "I lowered n_processes to "
                    + str(function_args["n_processes"])
                    + "."
                )

            # Start the paralel calculation
            parallel_calculation = rp_parallel.parallel(
                function_args["n_processes"]
            )
            result_process = parallel_calculation.calculate(
                self.results_directory, function_name, function_args
            )
            ...
            for key in result_process[0].keys():
                ...
                for process in range(1, function_args["n_processes"]):
                    try:
                        combined = np.append(
                            combined, result_process[process][key], axis=0
                        )
                    except:
                        pass
        ...
        for param in params:
            ...
            if log_abundances:
                ...
                if (
                    ...
                    local_titles[param] = (
                        "$L\\left(" + local_titles[param][1:-1] + "\\right)$"
                    )
    ...
    def Posteriors(
        ...
        local_post = {
            param_names[i]: np.copy(self.equal_weighted_post[:, i])
            for i in inds
        }
        ...
        for param in params:
            if not f"{local_units[param]:latex}" == "$\\mathrm{}$":
                unit = (
                    "\\left[" + f"{local_units[param]:latex}"[1:-1] + "\\right]"
                )
            else:
                unit = ""
        ...
        for param in params:
            local_post[param] = self.units.unit_conv(
                param,
                retrieval_unit[param],
                local_units[param],
                local_post[param],
                printing=False,
            )
            local_truths[param] = self.units.unit_conv(
                param,
                retrieval_unit[param],
                local_units[param],
                local_truths[param],
        ...
        local_post, local_truths, local_titles = self.Scale_Posteriors(
            local_post,
            local_truths,
            local_titles,
            params,
            ...
            log_particle_radii=log_particle_radii,
        )
        ...
        ULU = [
            param
            for param in params
            if self.params[param]["prior_type"] == "ULU"
        ]

        # If wanted add the bond albedo and the equilibrium temperature to the plot
        if plot_bond is not None:
            params += ["Teq", "A_b"]
            A_Bond_true, T_equ_true = self.Plot_Ret_Bond_Albedo(
                *plot_bond[:-2],
                A_Bond_true=plot_bond[-1],
                T_equ_true=plot_bond[-2],
                save=True,
                bins=20,
                plot=False,
            )
            local_post["Teq"], local_post["A_b"] = (
                self.ret_opaque_T.copy(),
                self.A_Bond_ret.copy(),
            )
            local_truths["Teq"], local_truths["A_b"] = T_equ_true, A_Bond_true
            local_titles["Teq"], local_truths["A_b"] = (
                "$\mathrm{T_{eq,\,Planet}\left[\mathrm{K}\\right]}$",
                "$\mathrm{A_{B,\,Planet}}$",
            )

        if plot_corner:
            fig, axs = rp_posteriors.Corner(
                params,
                local_post,
                local_titles,
                local_truths,
                ...
                ULU=ULU if ULU != [] else None,
                ULU_lim=ULU_lim,
            )
            if save:
                plt.savefig(
                    self.results_directory + "Plots/plot_corner.pdf",
                    bbox_inches="tight",
                )
            else:
                return fig, axs
            ...
            for param in params:
                fig, axs = rp_posteriors.Posterior(
                    local_post[param],
                    local_titles[param],
                    local_truths[param],
                    quantiles1d=quantiles1d,
    ...
    def PT_Envelope(
        ...
        self.get_pt(
            skip=skip, n_processes=n_processes, reevaluate_PT=reevaluate_PT
        )

        # Unit conversions for the x and y scales of the graph
        retrieval_units = {"x_unit": u.K, "y_unit": u.bar}
        local_units = {
            "x_unit": retrieval_units["x_unit"] if x_unit is None else x_unit,
            "y_unit": retrieval_units["y_unit"] if y_unit is None else y_unit,
        }
        unit_titles = {
            i: "$\\left[" + f"{local_units[i]:latex}"[1:-1] + "\\right]$"
            for i in local_units
        }

        # Convert the units of the P-T profile posteriors and the true value
        local_pressure_extrapol = self.units.unit_conv(
            "Pressure",
            retrieval_units["y_unit"],
            local_units["y_unit"],
            self.pressure_extrapol,
            printing=False,
        )
        local_temperature_extrapol = self.units.unit_conv(
            "Temperature",
            retrieval_units["x_unit"],
            local_units["x_unit"],
            self.temperature_extrapol,
            printing=False,
        )
        local_pressure = self.units.unit_conv(
            "Pressure",
            retrieval_units["y_unit"],
            local_units["y_unit"],
            self.pressure,
        ...
        local_temperature = self.units.unit_conv(
            "Temperature",
            retrieval_units["x_unit"],
            local_units["x_unit"],
            self.temperature,
        ...
        local_input_pressure = self.units.unit_conv(
            "Pressure",
            retrieval_units["y_unit"],
            local_units["y_unit"],
            self.input_pressure,
        ...
        local_input_temperature = self.units.unit_conv(
            "Temperature",
            retrieval_units["x_unit"],
            local_units["x_unit"],
        ...
        if self.settings["clouds"] == "opaque":
            local_pressure_cloud_top = self.units.unit_conv(
                "Pressure",
                retrieval_units["y_unit"],
                local_units["y_unit"],
                self.pressure_cloud_top,
                printing=False,
            )
            local_temperature_cloud_top = self.units.unit_conv(
                "Temperature",
                retrieval_units["x_unit"],
                local_units["x_unit"],
                self.temperature_cloud_top,
                printing=False,
            )
        try:
            local_true_pressure_cloud_top = self.units.unit_conv(
                "Pressure",
                retrieval_units["y_unit"],
                local_units["y_unit"],
                self.true_pressure_cloud_top,
                printing=False,
            )
            local_true_temperature_cloud_top = self.units.unit_conv(
                "Temperature",
                retrieval_units["x_unit"],
                local_units["x_unit"],
                self.true_temperature_cloud_top,
                printing=False,
            )
        except:
            local_true_temperature_cloud_top, local_true_pressure_cloud_top = (
                true_cloud_top[1],
                true_cloud_top[0],
            )
        ...
        p_layers_quantiles = [
            np.nanquantile(local_pressure_extrapol, q, axis=0)
            for q in quantiles
        ]
        if plot_residual:
            T_layers_quantiles = [
                np.nanquantile(local_temperature_extrapol, q, axis=0)
                - np.nanquantile(local_temperature_extrapol, 0.5, axis=0)
                for q in quantiles
            ]
        else:
            T_layers_quantiles = [
        ...
        if not self.settings["clouds"] == "opaque":
            ...
            t_lim = [
                np.min(local_temperature[:, -1]) - mean_S_T,
                np.max(local_temperature[:, -1]) - mean_S_T,
            ]
            t_range = t_lim[1] - t_lim[0]
            p_lim = [
                np.min(np.log10(local_pressure[:, -1])),
                np.max(np.log10(local_pressure[:, -1])),
            ]
            p_range = p_lim[1] - p_lim[0]

            # Calculate Contours for the surface pressure
            Z, X, Y = np.histogram2d(
                local_temperature[:, -1] - mean_S_T,
                np.log10(local_pressure[:, -1]),
                bins=100,
                range=[
                    [t_lim[0] - 0.1 * t_range, t_lim[1] + 0.1 * t_range],
                    [p_lim[0] - 0.1 * p_range, p_lim[1] + 0.1 * p_range],
                ],
            )
            Z = sp.ndimage.filters.gaussian_filter(Z, [7, 7], mode="reflect")
            color_levels, level_thresholds, N_levels = rp_col.color_levels(
                color, quantiles
            )
            map, norm, levels = rp_col.color_map(
            ...
            for i in range(len(contour)):
                ...
                dist = sp.spatial.distance.cdist(
                    np.array(
                        [
                            contour[i][0][:, 0] / 1000,
                            (np.log10(contour[i][0][:, 1]) + 6) / 10,
                        ]
                    ).T,
                    np.array(
                        [
                            T_layers_quantiles[-(i + 1)] / 1000,
                            (np.log10(p_layers_quantiles[-(i + 1)]) + 6) / 10,
                        ]
                    ).T,
                )
                dist2 = sp.spatial.distance.cdist(
                    np.array(
                        [
                            contour[i][0][:, 0] / 1000,
                            (np.log10(contour[i][0][:, 1]) + 6) / 10,
                    ...
                    np.array(
                        [
                            T_layers_quantiles[i] / 1000,
                            (np.log10(p_layers_quantiles[i]) + 6) / 10,
                        ]
                    ).T,
                )
                ...
                ind = np.array(
                    [
                        [i // s[1], i % s[1]]
                        for i in np.argsort(dist, axis=None)[:num]
                    ]
                )
                ind2 = np.array(
                    [
                        [i // s2[1], i % s2[1]]
                        for i in np.argsort(dist2, axis=None)[:num]
                    ]
                )
                ind = [
                    ind[np.argmax(p_layers_quantiles[-(i + 1)][ind[:, 1]])],
                    ind[np.argmin(p_layers_quantiles[-(i + 1)][ind[:, 1]])],
                ]
                ind2 = [
                    ind2[np.argmax(p_layers_quantiles[i][ind2[:, 1]])],
                    ind2[np.argmin(p_layers_quantiles[i][ind2[:, 1]])],
                ]
                ...
                p_layers_quantiles[-(i + 1)] = p_layers_quantiles[-(i + 1)][
                    : ind[0][1]
                ]
                T_layers_quantiles[-(i + 1)] = T_layers_quantiles[-(i + 1)][
                    : ind[0][1]
                ]
        ...
        if plot_clouds:
            cloud_top_quantiles = [
                np.quantile(local_pressure_cloud_top, q) for q in quantiles
            ]
        ...
        color_levels_c, level_thresholds_c, N_levels_c = rp_col.color_levels(
            "#898989", quantiles
        )
        ...
        if plot_clouds:
            for i in range(N_levels_c):
                ax.fill(
                    ...
                    [
                        cloud_top_quantiles[i],
                        cloud_top_quantiles[i],
                        cloud_top_quantiles[-i - 1],
                        cloud_top_quantiles[-i - 1],
                    ],
                    color=tuple(color_levels_c[i, :]),
                    clip_box=True,
                    zorder=-1,
                )
            for i in range(N_levels_c):
                ax.hlines(
                    [cloud_top_quantiles[i], cloud_top_quantiles[-i - 1]],
                    xmin=-10000,
                    xmax=10000,
                    color=tuple(color_levels_c[i, :]),
                    ls="-",
            ...
            if self.settings["clouds"] == "opaque":
                    ...
                    np.append(
                        np.append(
                            sp.ndimage.filters.gaussian_filter1d(
                                T_layers_quantiles[i], 5, mode="nearest"
                            ),
                            T_layers_bottom[i][::-1],
                        ),
                        np.flip(
                            sp.ndimage.filters.gaussian_filter1d(
                                T_layers_quantiles[-i - 1], 5, mode="nearest"
                            )
                        ),
                    ),
                    np.append(
                        np.append(
                            p_layers_quantiles[i], p_layers_bottom[i][::-1]
                        ),
                        np.flip(p_layers_quantiles[-i - 1]),
                    ),
                    color=tuple(color_levels[i, :]),
                    lw=0,
                    clip_box=True,
                    zorder=1,
                )
                ...
                ax.fill(
                    sp.ndimage.filters.gaussian_filter1d(
                        np.append(
                            np.append(
                                T_layers_quantiles[i], T_layers_bottom[i][::-1]
                            ),
                            np.flip(T_layers_quantiles[-i - 1]),
                        ),
                        10,
                        mode="nearest",
                    ),
        ...
        if plot_residual:
            ...
            ax.annotate(
                ...
                [
                    ...
                    10
                    ** (
                        0.975 * (np.log10(y_lim[1]) - np.log10(y_lim[0]))
                        + np.log10(y_lim[0])
                    ),
                ],
                color="black",
        ...
        if plot_residual:
            ...
            smooth_T_true = sp.ndimage.filters.gaussian_filter1d(
                local_input_temperature - yinterp, sigma=5
            )
            ...
            if np.isnan(smooth_T_true[-10]):
                ...
                ax.semilogy(
                    smooth_T_true[: -num_nan - 30],
                    local_input_pressure[: -num_nan - 30],
                    color="black",
                    label="P-T Profile",
                )
                ax.semilogy(
                ...
                ax.plot(
                    local_input_temperature[-1] - yinterp[-1],
                    local_input_pressure[-1],
                    marker="s",
                    color="C3",
                    ms=7,
            ...
            try:
                ind_ct = np.argmin(
                    np.abs(
                        np.log10(local_true_pressure_cloud_top)
                        - np.log10(local_input_pressure)
                    )
                )
                ax.plot(
                    smooth_T_true[ind_ct],
                    local_true_pressure_cloud_top,
                    marker="o",
                    color="C1",
                    lw=0,
                    ms=7,
            ...
            try:
                ax.plot(
                    local_true_temperature_cloud_top,
                    local_true_pressure_cloud_top,
                    marker="o",
                    color="C1",
                    lw=0,
        ...
        if ax_arg is None:
            if plot_residual:
                ax.set_xlabel(
                    "Difference to Retrieved Median " + unit_titles["x_unit"]
                )
            else:
        ...
        ax2 = rp_inlay.position_inlay(
            inlay_loc, figure, ax_arg, ax, h_cover=h_cover
        )
        ...
        if self.settings["clouds"] == "opaque":
            ...
            ax2_xlabel = (
                "$T^\mathrm{cloud}_\mathrm{top}$ " + unit_titles["x_unit"]
            )
            ax2_ylabel = (
                "$P^\mathrm{cloud}_\mathrm{top}$ " + unit_titles["y_unit"]
            )

            # Define limits and make a 2d histogram of the cloud top pressures and temperatures
            t_lim = [
                np.min(local_temperature_cloud_top),
                np.max(local_temperature_cloud_top),
            ]
            t_range = t_lim[1] - t_lim[0]
            p_lim = [
                np.min(np.log10(local_pressure_cloud_top)),
                np.max(np.log10(local_pressure_cloud_top)),
            ]
            p_range = p_lim[1] - p_lim[0]
            Z, X, Y = np.histogram2d(
                local_temperature_cloud_top[:, 0],
                np.log10(local_pressure_cloud_top)[:, 0],
                bins=bins_inlay,
                range=[
                    [t_lim[0] - 0.1 * t_range, t_lim[1] + 0.1 * t_range],
                    [p_lim[0] - 0.1 * p_range, p_lim[1] + 0.1 * p_range],
            ...
            # Define limits and make a 2d histogram of the surface pressures and temperatures
            t_lim = [
                np.min(local_temperature[:, -1]),
                np.max(local_temperature[:, -1]),
            ]
            t_range = t_lim[1] - t_lim[0]
            p_lim = [
                np.min(np.log10(local_pressure[:, -1])),
        ...
        t_lim = [
            np.min(
                [
                    np.min(contour.allsegs[0][i][:, 0])
                    for i in range(len(contour.allsegs[0]))
                ]
            ),
            np.max(
                [
                    np.max(contour.allsegs[0][i][:, 0])
                    for i in range(len(contour.allsegs[0]))
                ]
            ),
        ]
        p_lim = [
            np.min(
                [
                    np.min(contour.allsegs[0][i][:, 1])
                    for i in range(len(contour.allsegs[0]))
                ]
            ),
            np.max(
                [
                    np.max(contour.allsegs[0][i][:, 1])
                    for i in range(len(contour.allsegs[0]))
                ]
            ),
        ]
        ...
        if x_lim_inlay is None:
            ...
            ax2_xlim = [
                t_lim[0] - 0.1 * (t_lim[1] - t_lim[0]),
                t_lim[1] + 0.1 * (t_lim[1] - t_lim[0]),
            ]
        else:
        ...
        if y_lim_inlay is None:
            if np.log10(p_lim[1]) - np.log10(p_lim[0]) >= 1.2:
                ...
                ax2_ylim = [
                    ...
                    ** (
                        np.log10(p_lim[0])
                        - 0.1 * (np.log10(p_lim[1]) - np.log10(p_lim[0]))
                    ),
                    10
                    ** (
                        np.log10(p_lim[1])
                ...
                log_p = False
                ax2_ylim = [
                    max([p_lim[0] - 0.1 * (p_lim[1] - p_lim[0]), 0]),
                    p_lim[1] + 0.1 * (p_lim[1] - p_lim[0]),
                ]
        else:
        ...
        xticks = np.array(
            [
                (1 - pos) * ax2_xlim[0] + pos * ax2_xlim[1]
        ...
        if roundx >= 0.5:
            ...
            ax2.set_xticklabels(
                np.round(xticks, int(-np.floor(roundx - 0.5))), rotation=90
            )
        ...
        if log_p:
            ...
            yticks = [
                10**i for i in range(log_range[0] + 1, log_range[1] + 1)
            ]
            ax2.set_yticks(yticks)
        else:
            yticks = np.array(
                [
                    (1 - pos) * ax2_ylim[0] + pos * ax2_ylim[1]
                    for pos in [0.2, 0.4, 0.6, 0.8]
                ]
            )
            ...
            if roundy >= 0.5:
                ...
                ax2.set_yticklabels(
                    np.round(yticks, int(-np.floor(roundy - 0.5)))
                )
        ...
        if plot_clouds:
            patch_handles = [
                rp_hndl.MulticolorPatch(
                    [tuple(color_levels[i, :]), tuple(3 * [0.9 - i * 0.15])],
                    [1, 1],
                )
                for i in range(N_levels)
            ]
            ...
            patch_handles = [
                rp_hndl.MulticolorPatch([tuple(color_levels[i, :])], [1])
                for i in range(N_levels)
            ]
        ...
        if quantiles_title is None:
            patch_labels = [
                str(quantiles[i]) + "-" + str(quantiles[-i - 1])
                for i in range(N_levels)
            ]
        else:
        ...
        if case_identifier == "":
            lgd = ax.legend(
                ["Retrieval:"] + patch_handles + [" ", "Truth:"] + handles,
                [" "] + patch_labels + [" ", " "] + labels,
                handler_map={
                    str: rp_hndl.Handles(),
                    rp_hndl.MulticolorPatch: rp_hndl.MulticolorPatchHandler(),
                },
                ncol=legend_n_col,
                loc=legend_loc,
                frameon=False,
        ...
        if ax_arg is not None:
            ...
            if plot_residual:
                plt.savefig(
                    self.results_directory
                    + "Plots/plot_pt_structure_residual.pdf",
                    bbox_inches="tight",
                ...
                plt.savefig(
                    self.results_directory + "Plots/plot_pt_structure.pdf",
                    bbox_inches="tight",
                    bbox_extra_artists=(lgd,),
                    transparent=True,
                )
    ...
    def PT_Histogram(
        ...
        bins = np.array(
            [
                np.linspace(x_lim[0], x_lim[1], bins),
                np.logspace(np.log10(y_lim[0]), np.log10(y_lim[1]), bins),
            ]
        )
        ax.hist2d(
            self.temperature_extrapol.flatten()[
                ~np.isnan(self.temperature_extrapol.flatten())
            ],
            self.pressure_extrapol.flatten()[
                ~np.isnan(self.pressure_extrapol.flatten())
            ],
            bins=bins,
            cmap=color_map,
        )
        ...
        # Plotting the true/input surface temperature/pressure
        ax.plot(
            self.input_temperature[-1],
            self.input_pressure[-1],
            marker="s",
            color="C3",
            ms=7,
        ...
        if ax_arg is not None:
            ...
            plt.savefig(
                self.results_directory + "Plots/plot_pt_histogram.pdf",
                bbox_inches="tight",
                bbox_extra_artists=(lgd,),
            )
        return figure, ax
    ...
    def Flux_Error(
        ...
        self.get_spectra(
            skip=skip,
            n_processes=n_processes,
            reevaluate_spectra=reevaluate_spectra,
        )

        # Unit conversions for the x and y scales of the graph
        retrieval_units = {
            "x_unit": self.units.retrieval_units["wavelength"],
            "y_unit": self.units.retrieval_units["flux"],
        }
        local_units = {
            "x_unit": retrieval_units["x_unit"] if x_unit is None else x_unit,
            "y_unit": retrieval_units["y_unit"] if y_unit is None else y_unit,
        ...
        for instrument in local_instrument:
            conv_data = self.units.unit_spectrum_conv(
                "spec",
                [retrieval_units["x_unit"], retrieval_units["y_unit"]],
                [local_units["x_unit"], local_units["y_unit"]],
                np.array(
                    [
                        self.instrument[instrument]["wl"],
                        self.instrument[instrument]["flux"],
                        self.instrument[instrument]["error"],
                    ]
                ).T,
                printing=False,
            )
            (
                local_instrument[instrument]["wl"],
                local_instrument[instrument]["flux"],
                local_instrument[instrument]["error"],
            ) = (conv_data[:, 0], conv_data[:, 1], conv_data[:, 2])
        local_wavelength, local_fluxes = self.units.unit_spectrum_cube(
            [retrieval_units["x_unit"], retrieval_units["y_unit"]],
            [local_units["x_unit"], local_units["y_unit"]],
            self.wavelength,
            self.retrieved_fluxes,
        )

        # If provided select instruments to plot
        if (not single_instrument is None) and (
            not single_instrument in local_instrument.keys()
        ):
            print(
                single_instrument
                + " is not a valid instrument. Valid instruments:",
                list(local_instrument.keys()),
            )
            sys.exit()
        intruments = (
            local_instrument.keys()
        ...
        fac_resid = {
            inst: (100 / local_instrument[inst]["flux"] if plot_residual else 1)
        ...
        quantiles_all_wl = [
            np.quantile(local_fluxes, q, axis=0) for q in quantiles
        ]
        ...
        if split_instruments or plot_residual:
            for inst in intruments:
                ...
                if not np.array(
                    [
                        (
                            np.round(local_instrument[inst]["wl"], 10)
                            == np.round(local_wavelength, 10)
                        )
                    ]
                ).all():
                    inst_median[inst] = (
                        spectres.spectres(
                            local_instrument[inst]["wl"],
                            local_wavelength,
                            median_all_wl,
                        )
                        - local_instrument[inst]["flux"] * fac_input
                    ) * fac_resid[inst]
                    inst_quantiles[inst] = [
                        (
                            spectres.spectres(
        ...
        for inst in inst_wls.keys():
            if median_only:
                ...
                for i in range(N_levels):
                    ax.fill(
                        np.append(inst_wls[inst], np.flip(inst_wls[inst])),
                        np.append(
                            inst_quantiles[inst][i],
                            np.flip(inst_quantiles[inst][-i - 1]),
                        ),
                        color=tuple(color_levels[i, :]),
                        lw=0,
                        clip_box=True,
        ...
        for inst in intruments:
            ...
            if plot_noise:
                ax.fill(
                    np.append(
                        local_instrument[inst]["wl"],
                        np.flip(local_instrument[inst]["wl"]),
                    ),
                    np.append(
                        (
                            local_instrument[inst]["flux"] * abs(fac_input - 1)
                            + local_instrument[inst]["error"]
                        )
                        * fac_resid[inst],
                        np.flip(
                            (
                                local_instrument[inst]["flux"]
                                * abs(fac_input - 1)
                                - local_instrument[inst]["error"]
                            )
                            * fac_resid[inst]
            ...
            if plot_datapoints:
                ax.errorbar(
                    local_instrument[inst]["wl"],
                    local_instrument[inst]["flux"] * abs(fac_input - 1),
                    yerr=local_instrument[inst]["error"],
                    color="k",
                    ms=3,
                    marker="o",
                    ls="",
        ...
        if x_lim is not None:
            ...
            for inst in inst_wls.keys():
                x_lim = [
                    min(inst_wls[inst][0], x_lim[0]),
                    max(inst_wls[inst][-1], x_lim[1]),
                ]
            ax.set_xlim(x_lim)
        ...
        if case_identifier is not None:
            if plot_residual:
                ax.annotate(
                    case_identifier,
                    [
                        x_lim[1] - 0.025 * (x_lim[1] - x_lim[0]),
                        y_lim[0] + 0.1 * (y_lim[1] - y_lim[0]),
                    ],
                    ha="right",
                    va="bottom",
                    weight="bold",
        ...
        if not median_only:
            ...
            if plot_noise:
                patch_handles = [
                    rp_hndl.MulticolorPatch([(0.8, 0.8, 0.8)], [1])
                ] + patch_handles
    ...
    def Plot_Ret_Bond_Albedo(
        ...
        titles=[
            r"$\mathrm{L_{Star}}$",
            r"$\mathrm{a_{Planet}}$",
            r"$\mathrm{T_{eq,\,Planet}}$",
            r"$\mathrm{A_{B,\,Planet}}$",
        ],
        units=[
            r"$\left[\mathrm{L}_\odot\right]$",
            r"$\left[\mathrm{AU}\right]$",
            r"$\left[\mathrm{K}\right]$",
            "",
        ],
        reevaluate_PT=False,
        reevaluate_spectra=False,
                ...
                / (
                    ...
                    * (
                        np.exp(
                            sp.constants.h
                            * sp.constants.c
                            / (lam * sp.constants.k * T)
                        )
                        - 1
                    )
                )
            )  # calculate the BB flux
            return [np.sum(flux)]

        ind_r = [
            i
            for i in range(len(list(self.params.keys())))
            if list(self.params.keys())[i] == "R_pl"
        ][0]
        ...
        for i in range(np.size(self.retrieved_fluxes[:, 0])):
            factor = (
                1e7
                / 1e6
                / (self.nc.c / self.wavelength * 1e4)
                * 1e6
                * self.wavelength
                * 1e-6
                * (self.equal_weighted_post[i, ind_r]) ** 2
                / (self.knowns["d_syst"]["value"]) ** 2
            )
            self.ret_opaque_T[i, 0], cov = sp.optimize.curve_fit(
                blackbody_lam,
                [1],
                np.sum(np.ndarray.flatten(self.retrieved_fluxes[i]) / factor),
                p0=[300],
            )
        ...
        L_star_data = L_star + sigma_L_star * np.random.randn(
            *self.ret_opaque_T.shape
        )
        sep_planet_data = sep_planet + sigma_sep_planet * np.random.randn(
            *self.ret_opaque_T.shape
        )
        ...
        self.A_Bond_ret = (
            ...
            - 16
            * np.pi
            * sep_planet_data_SI**2
            * sigma_SBoltzmann
            * self.ret_opaque_T**4
            / L_star_data_SI
        )
        A_Bond_true = 1 - 16 * np.pi * (
            sep_planet * AU
        ) ** 2 * sigma_SBoltzmann * T_equ_true**4 / (L_star * L_sun)
        ...
        if plot:
            ...
            fig, axs = rp_posteriors.Corner(
                data,
                titles,
                [L_star, sep_planet, T_equ_true, A_Bond_true],
                bins=bins,
                quantiles1d=quantiles1d,
            )
            ...
            if save:
                plt.savefig(
                    self.results_directory + "Plots/plot_bond_albedo.pdf",
                    bbox_inches="tight",
                )
    ...
    def Emission_Contribution(
        ...
        local_retrieved_em_contr = np.sum(
            (local_retrieved_em_contr * self.retrieved_fluxes[:, None, :])
            / (np.sum(self.retrieved_fluxes, axis=1)[:, None, None]),
            axis=0,
        )
        ...
        if self.settings["clouds"] == "opaque":
            p = np.linspace(
                np.log10(self.pressure_extrapol[0, 0]),
                np.log10(self.pressure_extrapol[0, -1]),
                100,
            )
            pwidth = (p[1] - p[0]) / 2

            local_retrieved_em_contr = np.mean(
                local_retrieved_em_contr, axis=1
            ) / np.sum(np.mean(local_retrieved_em_contr, axis=1))
            for i in range(len(p)):
                plt.fill(
                    ...
                    color="k",
                    alpha=local_retrieved_em_contr[i]
                    / np.max(local_retrieved_em_contr),
                    edgecolor=None,
                )
        else:
            ...
            for i in range(len(local_retrieved_em_contr[0, :])):
                ...
                p_in = np.linspace(
                    np.log10(self.pressure[i, 0]),
                    np.log10(self.pressure[i, -1]),
                    100,
                )
                f = sp.interpolate.interp1d(
                    p_in,
                    local_retrieved_em_contr[:, i],
                    fill_value=0,
                    bounds_error=False,
                )
        ...
        plt.savefig(
            self.results_directory + "Plots/Mean_Emission_Contribution.pdf"
        )

    # TO DO: Add multiple instrument handling
    def Emission_Contribution_Wlen(
        self,
        skip=1,
        n_processes=50,
        color="k",
        ...
        local_retrieved_em_contr = self.retrieved_em_contr.copy()
        local_retrieved_em_contr = (
            local_retrieved_em_contr[:, ::2, :]
            + local_retrieved_em_contr[:, 1::2, :]
        )
        local_retrieved_em_contr = (
            local_retrieved_em_contr[:, ::2, :]
            + local_retrieved_em_contr[:, 1::2, :]
        ...
        cmap = col.LinearSegmentedColormap.from_list(
            "my_list", ["w", color], N=1000
        )

        plt.figure(figsize=(2.2, 1.4))
        if self.settings["clouds"] == "opaque":
            local_retrieved_em_contr = np.mean(
                local_retrieved_em_contr, axis=0
            ) / (
                np.sum(np.mean(local_retrieved_em_contr, axis=0), axis=0)[
                    None, :
                ]
            )
            local_retrieved_em_contr = local_retrieved_em_contr / np.max(
                local_retrieved_em_contr
            )
            plt.contourf(
                X,
        ...
        plt.savefig(
            self.results_directory
            + "Plots/Mean_Emission_Contribution_Wlen.pdf",
            bbox_inches="tight",
            transparent=True,
        )
    ...
    def Posterior_Classification(
        self,
        parameters=["H2O", "CO2", "CO", "H2SO484(c)", "R_pl", "M_pl"],
        relative=None,
        limits=None,
        ...
        for param in parameters:
            if True:  # try:
                ...
                if limits is None:
                    ...
                    if prior in ["ULU", "LU"]:
                        if prior == "LU":
                            ...
                            self.best_post_limit[param] = sorted(
                                list(self.priors_range[ind])
                            )
                        else:
                            if relative is None:
                ...
                span = abs(
                    self.best_post_limit[param][1]
                    - self.best_post_limit[param][0]
                )
                ...
                binned_data = np.histogram(
                    post,
                    bins=100,
                    range=self.best_post_limit[param],
                    density=True,
                )
                ...
                try:
                    ...
                    model_likelihood.append(
                        r_post.log_likelihood(
                            params_F,
                            (binned_data[1][:-1] + binned_data[1][1:]) / 2,
                            binned_data[0],
                            r_post.model_flat,
                ...
                if model_likelihood[2] != -np.inf:
                    if (
                        np.max(line_SSG) <= 1.4 * params_SSG[2]
                        or np.max(line_SSG) >= 15 * params_SSG[2]
                    ):
    ...
    def Moon_flux(self, MoonDIR):
        ...
        # get wl range
        Earth_Spec = np.loadtxt(
            "{}input_spectrum.txt".format(DIR), delimiter=" "
        )
        L = Earth_Spec[:, 0]
        f = self.nc.c / (L * 1e-4)
        ...
        plt.fill_between(
            L, mf_q5, mf_q95, facecolor="C2", alpha=0.1, label="0.05-0.95"
        )
        plt.fill_between(
            L, mf_q15, mf_q85, facecolor="C2", alpha=0.3, label="0.15-0.85"
        )
        plt.fill_between(
            L, mf_q25, mf_q75, facecolor="C2", alpha=0.5, label="0.25-0.75"
        )
        plt.fill_between(
            L, mf_q35, mf_q65, facecolor="C2", alpha=0.7, label="0.35-0.65"
        )
    ...
    def Plot_Ice_Surface_Test(
        ...
        T, p_part = np.loadtxt(
            self.code_directory
            + "/retrieval_plotting_support/vapour_pressure_data/ice.dat"
        ).T
        ...
        for i in range(len(ret_surface_T)):
            ...
            p_part_fit[i] = p_part[ind[0]] - (T[ind[0]] - ret_surface_T[i]) * (
                p_part[ind[0]] - p_part[ind[1]]
            ) / (T[ind[0]] - T[ind[1]])
        ...
        if ax_arg is not None:
            ...
            ax.legend(loc="best")
            plt.savefig(
                self.results_directory + "Plots/Ice_Surface_Exclusion.png",
                dpi=450,
            )
        else:
            ax.legend(loc="best")
