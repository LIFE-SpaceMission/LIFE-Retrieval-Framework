...
if __name__ == "__main__":
    titl = ['','','','',r'$R_\mathrm{pl}[R_\oplus]$',r'$L(R_\mathrm{pl}[M_\oplus])$',r'$L(1-\mathrm{CO_2})$',r'$L(\mathrm{H_2O})$',r'$L(\mathrm{CO})$',r'$L(\mathrm{H_2SO_4^{cloud}})$',r'$L(P^\mathrm{cloud}_\mathrm{top}[\mathrm{bar}])$',r'$L(P^\mathrm{cloud}_\mathrm{span}[\mathrm{bar}])$',r'$L(\bar{R}^\mathrm{cloud}[\mathrm{cm}])$',r'$\sigma^\mathrm{cloud}$',r'$T_\mathrm{equ}[\mathrm{K}]$',r'$A_\mathrm{B}$']
    from retrieval_plotting_support import retrieval_plotting_adjust_config as rp_config


    #results =  retrieval_plotting('/net/ipa-gate/export/ipa/quanz/user_accounts/konradb/chains/Validation_R1000/')

    #results =  retrieval_plotting('/net/ipa-gate/export/ipa/quanz/user_accounts/konradb/Venus/Validation_R1000/')

    #results.get_fluxes(skip=1,n_processes=100)
    #results.PT_Envelope(save =True, plot_clouds = False,loc_surface='top right',skip=1)
    #results.Flux_Error(skip =1,save =True,quantiles=[0.05,0.15,0.25,0.35,0.65,0.75,0.85,0.95])
    #results.Corner_Plot(save=True, log_pressures=True, log_mass=True, log_abundances=True, log_particle_radii=True, units=None, bins=50,
    #                    quantiles=[0.16, 0.5, 0.84], precision=3, plot_pt=False,titles=titl,plot_bond=[1,0.05,0.723,0.723*0.05,226,0.77])
    #results.Plot_Ret_Bond_Albedo(1,0.1,0.723,0.723*0.1,A_Bond_true = 0.77, T_equ_true=226,save = True,bins=20)

    #results.PT_Envelope(save=True, plot_clouds = False, color='C2', loc_surface='top right', x_lim =[0,400], y_lim = [1e-6,1e1],
    #                    quantiles=[0.05,0.15,0.25,0.35,0.65,0.75,0.85,0.95],skip=1)
    #results.PT_Envelope_Residual(save=True, plot_clouds = True, color='C2', loc_surface='top right', x_lim =[-20,20], y_lim = [1e-6,1e3],
    #                    quantiles=[0.05,0.15,0.25,0.35,0.65,0.75,0.85,0.95],skip=1)


    #results.Rel_Flux_Error(skip =1,save =True,quantiles=[0.05,0.15,0.25,0.35,0.65,0.75,0.85,0.95])
    #results.Flux_Error(skip=1,save = True,quantiles = [0.05,0.15,0.25,0.35,0.65,0.75,0.85,0.95])
    #results.Input_Flux(skip= 100,save=True,pptx=True)

    
    """
    case = ['4Waterclouds/3ezodis_Waterclouds_']#'1Opaque/3ezodis_','2Cloudfree/3ezodis_cloudfree_','3Transparent/3ezodis_transparent_','4Waterclouds/3ezodis_Waterclouds_']
    case_identifier = {'1Opaque/3ezodis_':r'Opaque $\mathrm{H_2SO_4}$','2Cloudfree/3ezodis_cloudfree_':r'Cloud-Free','3Transparent/3ezodis_transparent_':r'Transparent $\mathrm{H_2SO_4}$','4Waterclouds/3ezodis_Waterclouds_':r'Opaque $\mathrm{H_2O}$'}
    DIR = ''
    colors = {'1Opaque/3ezodis_':'#009e73','2Cloudfree/3ezodis_cloudfree_':'#e69f00','3Transparent/3ezodis_transparent_':'#0072b2','4Waterclouds/3ezodis_Waterclouds_':'#d55e00'}
    RGB_colors = {'1Opaque/3ezodis_':[0, 158, 115], '2Cloudfree/3ezodis_cloudfree_':[230, 159, 0], '3Transparent/3ezodis_transparent_':[0, 114, 178],'4Waterclouds/3ezodis_Waterclouds_':[213, 94, 0]}

    
    for c in range(len(case)):
        for l in ['3-20']:#,'4-18.5','6-17']:
            for sn in ['10']:#'5','10','15','20']:
                for r in ['50']:#'20','35','50','100']:
                    print(l,sn,r)
                    print(DIR)
                    DIR = '/net/ipa-gate/export/ipa/quanz/user_accounts/konradb/Venus/Venus_Grid_Results/'+str(case[c])+'SN'+sn+'_R'+r+'_'+l+'/'
                    Update_Config(DIR,'PATHS','settings_opacity_database','/../../../net/ipa-gate/export/ipa/quanz/user_accounts/konradb/input_data/')

                    results = retrieval_plotting(DIR)
                    #results.get_fluxes(skip=1,n_processes=50)
                    #results.Rel_Flux_Error(skip =1,save =True,quantiles=[0.05,0.15,0.25,0.35,0.65,0.75,0.85,0.95],y_lim=[-105,105],color = colors[case[c]], case_identifier=case_identifier[c])
                    if case[c] == '2Cloudfree/3ezodis_cloudfree_':
                        plot_clouds = False
                    else:
                        plot_clouds = True

                    results.PT_Envelope_Residual(save=True, plot_clouds = plot_clouds, color=colors[case[c]], loc_surface='top right', x_lim =[-55,55], y_lim = [1e-6,1e3],
                        quantiles=[0.05,0.15,0.25,0.35,0.65,0.75,0.85,0.95],quantiles_title=[r'$5-95\%$',r'$15-85\%$',r'$25-75\%$',r'$35-65\%$'],skip=1, case_identifier=case_identifier[case[c]])

                    #results.Corner_Plot(save=True, log_pressures=True, log_mass=True, log_abundances=True, log_particle_radii=True, units=None, bins=50,
                    #                quantiles=[0.16, 0.5, 0.84], precision=3, plot_pt=False,plot_bond=[1,0.05,0.723,0.723*0.05,226,0.77])
        #results.Plot_Ret_Bond_Albedo(1,0.1,0.723,0.723*0.1,A_Bond_true = 0.77, T_equ_true=226,save = True,bins=20)
    ...
    #gp = grid_plotting('/net/ipa-gate/export/ipa/quanz/user_accounts/konradb/Venus/Venus_Grid_Results/')
    #gp.Parameters_Hist_Top()
    #gp.PT_grid(plot_clouds=True, Hist=False)
    #gp.PT_grid(plot_clouds=True, Hist=True)
    #gp.Parameters_Comparison(parameters=['R_pl','M_pl','H2SO484(c)_am_top_pressure'], priors = [True, True,False], units=['$\mathrm{R_\oplus}$','dex','dex'], span = [0.2,0.5,0.5])
    #gp.Parameters_Comparison(parameters=['R_pl','M_pl','A_Bond','T_eq'], priors = [True, True,False,False], units=['$\mathrm{R_\oplus}$','dex','','K'], span = [0.1,0.5,0.1,20],filename='R_M_Bond_Teq')
    #gp.Model_Comparison()
    #gp.Model_Comparison_Wlen()
    #gp.Model_Comparison_All()
    #gp.Parameters_Hist()
    #gp.Spectrum_grid(skip=100)
    #gp.Spectrum_grid(skip=100,NoQuant=True)
    #gp.Constraint_Analysis()
    #gp.Ice_Surface()


    #if len(sys.argv) != 2:
    #	sys.stderr.write("""SYNOPSIS:  %s <output-root>\n
    #    \toutput-root:\tFolder where the output of a MultiNest run has been written to.\n
    #    \t\t\tExample: chains/run1\n""" % (sys.argv[0]))
    #	sys.exit(1)

    #model=sys.argv[1]+'/'
    #if not os.path.isdir(model):
    #    sys.exit('Error! Cannot find the folder.')


    #for i in ['No']+list(range(20)):
    #    print('Noise Realization '+str(i))
    #    results = retrieval_plotting('/net/ipa-gate/export/ipa/quanz/user_accounts/konradb/Earth/Noisy/Noise_Additional/Noise_'+str(i)+'/')


    #    results.Flux_Error(skip =50,save =True,pptx=True)
    #    results.Input_Flux(skip= 50,save=True,pptx=True)
    #results.Corner_Plot(save = True)
    #results.PT_Envelope(save =True, plot_clouds = False,loc_surface='top right',skip=1)
    #results.PT_Histogram(save=True,plot_clouds = False,skip=1)
    #results.Flux_Error_noQuant(skip= 20,save=True,pptx=True)
    #results.Rel_Flux_Error(skip = 50,save =True,pptx=True)
    #results.Rel_Flux_Error_noQuant(skip = 100,save =True)
    #results.Plot_Ret_Bond_Albedo(1,0.1,0.723,0.723*0.1,A_Bond_true = 0.77, T_equ_true=226,save = True)
    #results.Plot_Ret_Bond_Albedo(1,0.1,0.723,0.723*0.1,A_Bond_true = 0.3, T_equ_true=255,save = True)
    ...
    #results =  retrieval_plotting('/net/ipa-gate/export/ipa/quanz/user_accounts/konradb/Timmy_ML_Data/LIFE_R20_SNR5_wlen3_20')
    #results.get_fluxes(1)


    """
    case = ['1Opaque/3ezodis_','2Cloudfree/3ezodis_cloudfree_','3Transparent/3ezodis_transparent_','4Waterclouds/3ezodis_Waterclouds_']
    DIR = ''
    for l in ['3-20','4-18.5','6-17']:
        for sn in ['5','10','15','20']:
            for r in ['20','35','50','100']:
                print(l,sn,r)
                print(DIR)
                DIR = '/net/ipa-gate/export/ipa/quanz/user_accounts/konradb/Venus/Venus_Grid_Results/'+str(case[3])+'SN'+sn+'_R'+r+'_'+l+'/'
                Update_Config(DIR,'PATHS','settings_opacity_database','/../../../net/ipa-gate/export/ipa/quanz/user_accounts/konradb/input_data/')

                results = retrieval_plotting(DIR)
                results.get_fluxes(1)
                results.Rel_Flux_Error(skip =1,save =True,quantiles=[0.05,0.15,0.25,0.35,0.65,0.75,0.85,0.95],y_lim=[-4,4])
        #results.Corner_Plot(save=True, log_pressures=True, log_mass=True, log_abundances=True, log_particle_radii=True, units=None, bins=50,
        #                quantiles=[0.16, 0.5, 0.84], precision=3, plot_pt=False,plot_bond=[1,0.05,0.723,0.723*0.05,226,0.77])
        #results.Plot_Ret_Bond_Albedo(1,0.1,0.723,0.723*0.1,A_Bond_true = 0.77, T_equ_true=226,save = True,bins=20)
    ...
    DIR = '/net/ipa-gate/export/ipa/quanz/user_accounts/konradb/chains/VAE_var_p_full5d/'
    rp_config.Update_Config(DIR,'PATHS','settings_multinest','/')
    rp_config.Update_Config(DIR,'PATHS','settings_input_profile','/../../../net/ipa-gate/export/ipa/quanz/user_accounts/konradb/Retrieval_Inputs/true_PT_poly_Earth.txt')
    rp_config.Update_Config(DIR,'INPUT FILES','settings_data_file','/../../../net/ipa-gate/export/ipa/quanz/user_accounts/konradb/Retrieval_Inputs/Earth_Noisy/R50/true_spectrum.txt')
    
    
    results = retrieval_plotting('/net/ipa-gate/export/ipa/quanz/user_accounts/konradb/chains/VAE_var_p_full5d/')

    #results = retrieval_plotting('/net/ipa-gate/export/ipa/quanz/user_accounts/konradb/Earth/Noisy/Noise_Additional/Noise_No/')


    #results = retrieval_plotting('/net/ipa-gate/export/ipa/quanz/user_accounts/konradb/Venus/Validation_R1000/')
    #results.Corner_Plot(save=True, log_pressures=True, log_mass=True, log_abundances=True, log_particle_radii=False, units=None, bins=20,
    #                    quantiles=[0.16, 0.5, 0.84], precision=2)
    results.Corner_Plot(save=True, log_pressures=True, units=None, bins=20,
                        quantiles=[0.16, 0.5, 0.84], precision=2)
    results.PT_Envelope(save=True, plot_clouds = False, color='C0', loc_surface='top right', x_lim =[150,300], y_lim = [1e-6,1e1],
                        quantiles=[0.05,0.15,0.25,0.35,0.65,0.75,0.85,0.95],skip=10)
    results.Flux_Error(skip=1,save = True,quantiles = [0.05,0.15,0.25,0.35,0.65,0.75,0.85,0.95],n_processes=100)
    results.Rel_Flux_Error(skip =1,save =True,quantiles=[0.05,0.15,0.25,0.35,0.65,0.75,0.85,0.95])
    #results.PT_Envelope_Residual(save=True, plot_clouds = False, color='C2', loc_surface='top right', x_lim =[-20,20], y_lim = [1e-6,1e3],
    #                    quantiles=[0.05,0.15,0.25,0.35,0.65,0.75,0.85,0.95],skip=1)
